...
function StartScreen({showLoading, socket, setName, setStack, setGameURL}) {
                ...
                <div>
                    ...
                    <button className="bg-[#3EA66C] my-2 w-full h-16 sm:h-10 lg:h-12 xl:h-16 rounded-lg text-xl sm:text-lg lg:text-xl font-bold tracking-wide" 
                        onClick={(e) => {
                            ...
                            setName(user_name.current.value)
                            setStack(stack.current.value)
                            socket.emit("createMember&Room", {
