...
async def create_member_room(name,stack,ls_roomsId):
    ...
    return({"Status":"Success", "Message":"New room and player has been created"})
...
async def create_member(name, stack, roomId):
    ...
    if(len(playerRoomNames)!=0 ):  ## Used to check if the room exists
        if(len(playerQueueNames) != 0): ## Used to check if the player queue for that room exists
            ...
            with MongoClient(base_url) as client:
                msg_collection = client[dataBase][playerQueue] 
                msg_collection.insert_one({"queue" : [{"name" : name, "stack": stack,"roomId": roomId}], "roomId":roomId})  ## Adding a new collection for particular roomid
...
# API endpoint : allowPlayer :: Only owner can use this to allow players into the table
...
def startFirstRound(roomId, starting_board):  ## starts round and assigns card to players who are online and their seat number is assigned 
    ...
    with MongoClient(base_url) as client:
        ...
        "sideShowPossible":False, "currentPlayerBoard": temp_array1, "currentPlayerStack": ls_curr_stack, "game_board": starting_board}
...
def updateMove(roomId, name, move, amount = 0, playerseat = -22):
    with MongoClient(base_url) as client:
        ...
        if(move == "SeeCards"):
            ...
            elif(len(current_player_rotation)<=1):
